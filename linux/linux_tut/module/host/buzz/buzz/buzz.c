...
// Default AMBA AXI GPIO address (can be overridden via device tree)
#define GPIO_ADDR 0x41200000

static struct platform_device* buzz_pdev;
static void __iomem* gpio_base; // To access the AXI GPIO
...
// Function to initialize the GPIO controller
static int buzz_gpio_init(void) {
    struct resource* res;

    // Get platform resources (e.g., base address and size) from device tree
    res = platform_get_resource(buzz_pdev, IORESOURCE_MEM, 0);
    if (!res) {
        pr_err("Failed to get resource from device tree\n");
        return -ENOMEM;
    }

    gpio_base =
        ioremap(res->start, resource_size(res)); // Map GPIO memory region
    if (!gpio_base) {
        pr_err("Failed to map GPIO base address\n");
        return -ENOMEM;
    }

    pr_info("GPIO base mapped at: %p\n", gpio_base);
    return 0;
}

// Function to deinitialize the GPIO controller
static void buzz_gpio_deinit(void) {
    if (gpio_base) {
        iounmap(gpio_base);
    }
}

// Function to set GPIO value (PWM)
static void buzz_set_pwm(int freq, int time) {
    ...
    for (int i = 0; i < cycles; i++) {
        writel(1, gpio_base); // Set GPIO high
        mdelay(high_time);
        writel(0, gpio_base); // Set GPIO low
        mdelay(low_time);
    }
...
static int create_sysfs(void) {
    ...
    // Add start attribute
    error = sysfs_create_file(buzz_kobj, &start_attr.attr);
...
static int buzz_probe(struct platform_device* pdev) {
    ...
    // Initialize GPIO from device tree
    ret = buzz_gpio_init();
    if (ret) {
        pr_err("buzz: GPIO initialization failed\n");
        return ret;
    }
